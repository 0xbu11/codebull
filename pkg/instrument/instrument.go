...
func cloneVariableNames(variableNames []string) []string {
	if len(variableNames) == 0 {
		return nil
	}
	cloned := make([]string, len(variableNames))
	copy(cloned, variableNames)
	return cloned
}

func normalizeVariableNames(variableNames []string) []string {
	if len(variableNames) == 0 {
		return nil
	}

	seen := make(map[string]struct{}, len(variableNames))
	normalized := make([]string, 0, len(variableNames))
...
func setPCStatus(pc uint64, status PointStatus, line int, variableNames []string, collectStacktrace bool) {
	...
	if len(variableNames) > 0 {
		pointVariableNamesBy[pc] = cloneVariableNames(variableNames)
	} else {
...
func GetPointVariableNamesAtPC(pc uint64) ([]string, bool) {
	pcStatusMu.RLock()
	defer pcStatusMu.RUnlock()
	...
	variableNames, ok := pointVariableNamesBy[pc]
	if !ok || len(variableNames) == 0 {
		return nil, false
	}
